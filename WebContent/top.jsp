        ...
        <tr>
          <td class='menuData'>
            <div class='menu'>
			  <c:if test="${empty LoginOK}">
				<a href="<c:url value='${request.contextPath}/Member_SignUp/Member_Login.jsp' />">
				     登入 
				</a>
             </c:if>
			</div>
		  ...
		  <td class='menuData'>
			<div class='menu'>
			  <c:if test="${ funcName != 'SHO' }">
                   <a href="<c:url value='${request.contextPath}/RetrievePageProducts' />" >
				        商城 
				  </a>
			   </c:if>
			   <c:if test="${ funcName == 'SHO' }"> 
		  ...
		  <td class='menuData'>
			<div class='menu'>
			  <c:if test="${ funcName != 'CHE' }">
			     <a href="<c:url value='${request.contextPath}/MarketHome.jsp' />">
				     市場 
				 </a>
			  </c:if>
			  <c:if test="${ funcName == 'CHE' }"> 
		  ...
		  <td class='menuData'>
			<div class='menu'>
			  <c:if test="${ funcName != 'ORD' }">
				 <a href="<c:url value='${request.contextPath}/recipe_workpage.jsp' />">
				     食譜 
			     </a>
			  </c:if>
			  <c:if test="${ funcName == 'ORD' }"> 
		  ...
		  <td class='menuData'>
			<div class='menu'>
		      <c:if test="${ funcName != 'ACT' }">
				<a href="<c:url value='${request.contextPath}/ActiveHome.jsp' />">
                                             活動
                </a>
              </c:if>
			  <c:if test="${ funcName == 'ACT' }"> 
		  ...
		  <div class='menu'>
			<c:if test="${ funcName != 'REG' }">
			   <a href="<c:url value='${request.contextPath}/Member_SignUp/Member_SignUp.jsp' />"> 註冊 </a>
			</c:if> 
			<c:if test="${ funcName == 'REG' }"> 
                                                     註冊
            </c:if>
		  ...
		  <div class='menu' style='width:50px;'>
			<c:if test="${ funcName != 'IND' }">
			   <a href="<c:url value='${request.contextPath}/index.jsp' />"> 回首頁 </a>
			</c:if>
			</div>
		  </td>
