						...
						<div class="bg-pure-white" data-controller="dir">
							<div class="paywalled">
									...
									<div>
										...
										<section class="intro-container document-section">
											<h1
												...
												<c:if test="${login_ok!=null}">
													<c:choose>
														<c:when test="${flag!=true}">
															<span><i class="ion-ios-heart" id="heart"
																onclick="fun('${login_ok}','${list.rec_id}')"></i></span>
														</c:when>
														<c:otherwise>
															<span><i class="ion-ios-heart" id="heart"
																onclick="fun('${login_ok}','${list.rec_id}')"
												...
												<c:if test="${login_ok==null}">
													<span><i class="ion-ios-heart" id="heart"
														onclick="funLogin('${login_ok}','${list.rec_id}')"></i></span>
												</c:if>
											</h1>
											...
											<div class="media my-sm">
												<a href="#"> <span
													class="mr-sm float-left w-8 xs:w-auto"><img
														title="${datailBean.name}" class=" rounded-full"
														width="48" height="48"
														srcset="<c:url value='getImageDetail?mem_no=${detailBean.mem_no}'/>"
														src="" /></span> <span> <span class="block truncate"
														itemprop="author" itemscope
														itemtype="http://schema.org/Person"> <span
															itemprop="name"
															class="text-cookpad-14 xs:text-cookpad-16">${datailBean.name}</span>
													</span> <span
											...
											<div
												...
												data-clamp="2" data-clamp-more="更多"
												data-dir-attribute="auto" data-target="dir.auto">
								...
								<div
									...
									<span class="px-4"> <i class="icf icf--timer "></i>
										30min
									</span>
								</div>
										...
										<ol
											...
											<c:if test="${list.ingredients_A!=null}">
												<li class="ingredient py-1 border-quaternary border-dashed">
													<div itemprop="ingredients">
														<a
															href="<c:url value='/connectMall?mall_searchString=${list.ingredients_A}&mall_pageNo=1'/>">${list.ingredients_A}</a>
														${list.gram_A} g
													</div>
												</li>
											</c:if>
											<c:if test="${list.ingredients_B!=null}">
												<li class="ingredient py-1 border-quaternary border-dashed">
													<div itemprop="ingredients">
														<a
															href="<c:url value='/connectMall?mall_searchString=${list.ingredients_B}&mall_pageNo=1'/>">${list.ingredients_B}</a>
														${list.gram_B} g
													</div>
												</li>
											</c:if>
											<c:if test="${list.ingredients_C!=null}">
												<li class="ingredient py-1 border-quaternary border-dashed">
													<div itemprop="ingredients">
														<a
															href="<c:url value='/connectMall?mall_searchString=${list.ingredients_C}&mall_pageNo=1'/>">${list.ingredients_C}</a>
														${list.gram_C} g
													</div>
												</li>
											</c:if>
											<c:if test="${list.ingredients_D!=null}">
												<li class="ingredient py-1 border-quaternary border-dashed">
													<div itemprop="ingredients">
														<a
															href="<c:url value='/connectMall?mall_searchString=${list.ingredients_D}&mall_pageNo=1'/>">${list.ingredients_D}</a>
														${list.gram_D} g
													</div>
												</li>
											</c:if>
